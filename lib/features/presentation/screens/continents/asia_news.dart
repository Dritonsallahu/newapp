...
class AsiaNews extends StatefulWidget {
  const AsiaNews({Key? key}) : super(key: key);

  @override
  State<AsiaNews> createState() => _AsiaNewsState();
}

class _AsiaNewsState extends State<AsiaNews> {

  getAsiaNews(bool firstTime, String category) {
    var provider = Provider.of<PostProvider>(context, listen: false);
    if (!provider.getAsiaFechingStatus() && firstTime == true) {
      provider.getNewByContinentFromDB(context, "Asia",category);
      provider.setAsiaNewsFechingStatus(true);
    }
    else if (provider.getAsiaFechingStatus() && firstTime == false) {
      provider.getNewByContinentFromDB(context, "Asia", category);
    }
  }
        ...
        child: ListView(
          ...
          children: [
                              ...
                              Padding(
                                ...
                                child: GestureDetector(
                                  onTap: () {
                                    ...
                                    getAsiaNews(false, category.categoryName!);
                                  },
                                  child: Container(
                                      padding: const EdgeInsets.symmetric(
            ...
            ListView.builder(
                ...
                itemCount: postProvider.getNewsByContinent("Asia")!.length,
                itemBuilder: (context, index) {
                  var post = postProvider.getNewsByContinent("Asia")![index];
                  return CustomAnimatedList(
                    index: index,
