...
class UsaNews extends StatefulWidget {
  const UsaNews({Key? key}) : super(key: key);

  @override
  State<UsaNews> createState() => _UsaNewsState();
}

class _UsaNewsState extends State<UsaNews> {

  getUsaNews(bool firstTime, String category) {
    var provider = Provider.of<PostProvider>(context, listen: false);
    if (!provider.getUsaNewsFechingStatus() && firstTime == true) {
      provider.getNewByContinentFromDB(context, "USA",category);
      provider.setUsaNewsFechingStatus(true);
    }
    else if (provider.getUsaNewsFechingStatus() && firstTime == false) {
      provider.getNewByContinentFromDB(context, "USA", category);
    }
  }
        ...
        child: ListView(
          ...
          children: [
                  ...
                  child: Column(
                    children: [
                      ...
                      Row(
                        children: [
                          Text(
                            "News From All Over United States of Amerika",
                            style: GoogleFonts.inter(
                                fontSize: 15,
                              ...
                              Padding(
                                ...
                                child: GestureDetector(
                                  onTap: () {
                                    ...
                                    getUsaNews(false, category.categoryName!);
                                  },
                                  child: Container(
                                      padding: const EdgeInsets.symmetric(
            ...
            ListView.builder(
                ...
                itemCount: postProvider.getNewsByContinent("USA")!.length,
                itemBuilder: (context, index) {
                  var post = postProvider.getNewsByContinent("USA")![index];
                  return CustomAnimatedList(
                    index: index,
