...
class MiddleEastNews extends StatefulWidget {
  const MiddleEastNews({Key? key}) : super(key: key);

  @override
  State<MiddleEastNews> createState() => _MiddleEastNewsState();
}

class _MiddleEastNewsState extends State<MiddleEastNews> {

  getMiddleEastNews(bool firstTime, String category) {
    var provider = Provider.of<PostProvider>(context, listen: false);
    if (!provider.getMiddleEastNewsFechingStatus() && firstTime == true) {
      provider.getNewByContinentFromDB(context, "Middle East",category);
      provider.setMiddleEastNewsFechingStatus(true);
    }
    else if (provider.getMiddleEastNewsFechingStatus() && firstTime == false) {
      provider.getNewByContinentFromDB(context, "Middle East", category);
    }
  }
        ...
        child: ListView(
          ...
          children: [
                              ...
                              Padding(
                                ...
                                child: GestureDetector(
                                  onTap: () {
                                    ...
                                    getMiddleEastNews(false, category.categoryName!);
                                  },
                                  child: Container(
                                      padding: const EdgeInsets.symmetric(
            ...
            ListView.builder(
                ...
                itemCount: postProvider.getNewsByContinent("Middle East")!.length,
                itemBuilder: (context, index) {
                  var post = postProvider.getNewsByContinent("Middle East")![index];
                  return CustomAnimatedList(
                    index: index,
